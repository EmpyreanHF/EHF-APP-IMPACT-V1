    ...
    (function patchPostThread() {
        ...
        function _submitQuote() {
            ...
            if (_fbOk()) {
                var u = _us();
                try {
                    window.fbDb.collection('posts').add({
                        text: text,
                        quotedPostId: _activePostId || '',
                        authorId: u.id || '',
    ...
    (function patchMessagesFull() {
        ...
        _ready(function () {
            ...
            s.textContent = [
                ...
                '  display:flex;align-items:center;justify-content:center;',
                '  cursor:pointer;flex-shrink:0;font-size:1rem;',
                '  -webkit-tap-highlight-color:transparent;transition:background 0.15s,transform 0.12s;',
                '}',
                '#vf-msg-vn.recording{ background:#E41E3F;color:#fff;animation:vf-pulse 1s infinite; }',
                '#vf-msg-vn:active{ transform:scale(0.9); }',
                '@keyframes vf-pulse{ 0%,100%{box-shadow:0 0 0 0 rgba(228,30,63,0.4)} 50%{box-shadow:0 0 0 8px rgba(228,30,63,0)} }',
        ...
        function _injectComposer(container, peerId, peerName) {
            ...
            inp.addEventListener('input', function () {
                ...
                vnBtn.style.display   = hasText ? 'none' : 'flex';
            });
            inp.addEventListener('keydown', function (e) {
            ...
            vnBtn.setAttribute('aria-label','Voice note');
            vnBtn.innerHTML = '<i class="fas fa-microphone"></i>';
            ...
            vnBtn.addEventListener('click', function () {
                if (!_recording) {
                    ...
                    navigator.mediaDevices.getUserMedia({ audio: true }).then(function (stream) {
                        ...
                        _mediaRecorder.addEventListener('dataavailable', function (e) { _vnChunks.push(e.data); });
                        _mediaRecorder.addEventListener('stop', function () {
                            stream.getTracks().forEach(function (t) { t.stop(); });
                            var blob = new Blob(_vnChunks, { type: 'audio/webm' });
                            _sendVoiceNote(blob, peerId, peerName);
                        });
                        _mediaRecorder.start();
                        _recording = true;
                        vnBtn.classList.add('recording');
                        vnBtn.title = 'Stop recording';
                        vnBtn.innerHTML = '<i class="fas fa-stop"></i>';
                    }).catch(function () { _notify('Could not access microphone.', 'warning'); });
                } else {
                    /* Stop recording */
        ...
        function _doSend() {
            ...
            var vnBtn   = document.getElementById('vf-msg-vn');
